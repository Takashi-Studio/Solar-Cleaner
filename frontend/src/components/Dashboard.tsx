...
export const Dashboard: React.FC<DashboardProps> = ({ token, user, onLogout }) => {
  ...
  // احتساب الجهاز المختار ديناميكياً من مصفوفة الأجهزة لمنع stale closures وحلقات التحديث اللا نهائية
  ...
  // إدارة التحديد التلقائي والاختيار الافتراضي والبديل عند الحذف
  useEffect(() => {
    if (devices.length > 0) {
